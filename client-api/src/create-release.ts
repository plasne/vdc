...
cmd.option(
    ...
    .option(
        '-o, --organization <i>',
        '[REQUIRED] ORGANIZATION. The URL for the organization (ex. https://dev.azure.com/org).',
        parseInt
    )
    .option(
        '-t, --access-token <i>',
        '[REQUIRED] ACCESS_TOKEN. The Personal Access Token for an account that has access.',
        parseInt
    )
    .parse(process.argv);
...
async function setup() {
    try {
        ...
        logger.info(
            `ACCESS_TOKEN is "${ACCESS_TOKEN ? 'provided' : 'missing'}".`
        );
